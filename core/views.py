...
def maps_view(request, username=None):
    ...
    return render(
        request,
        "list-page.html",
        dict(item_name="map", title=_("Maps"), items=maps, params=params),
    )
...
def new_style_map_view(request, slug):
    ...
    return render(
        ...
        dict(
            ...
            + [[elem.polygon.title, elem.data] for elem in elements],
        ),
    )
    ...
    if level == 0:
        if title:
            elements = get_object_or_404(
                Polygon, level=level, title=title
            ).get_children()
        else:
        ...
        parent_title = args[-2]
        elements = get_object_or_404(
            Polygon, level=level, title=title, parent__title=parent_title
        ).get_children()
...
def polygon_export(request, pk):
    ...
    response = HttpResponse(content=geojson_data, content_type="text/plain")
    response["Content-Disposition"] = "attachment; filename={}.geojson".format(
        (element.title[:1].lower() + element.title[1:]).replace(".", "_")
    )
    return response
...
def add_map(request):
    ...
    regions = Polygon.objects.filter(lft__lte=F("rght") - 2).order_by(
        "tree_id", "level", "title"
    )
...
def get_polygons(request, parent_id):
    ...
    polygons = Polygon.objects.filter(**{filter_field: parent_id}).order_by("title")

    polygons = json.dumps(
        [{"pk": polygon.pk, "title": polygon.title} for polygon in polygons]
    )
...
def charts_view(request, username=None):
    ...
    return render(
        request,
        "list-page.html",
        dict(item_name="chart", title=_("Charts"), items=charts, params=params),
    )
...
def chart_view(request, slug):
    ...
    chart_obj = get_object_or_404(
        Chart.objects.prefetch_related(
            Prefetch(
                "maps",
                queryset=Map.objects.prefetch_related(
                    Prefetch(
                        "elements",
...
def plots_view(request, username=None):
    ...
    if username:
        user = get_object_or_404(User, username=username)
        plots = (
            Plot.objects.values("slug", "user")
            .annotate(date_of_information=Max("added"))
            .filter(user=user)
        )
    else:
        plots = (
    ...
    return render(
        request,
        "list-page.html",
        dict(item_name="plot", title=_("Plots"), items=plots, params=params),
    )
