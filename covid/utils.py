    ...
    if data:
        ...
        if province in mapping:
            stats[mapping[province]] = {
                "confirmed": stats.get(mapping[province], {}).get("confirmed", 0)
                + data[0],
                "deaths": stats.get(mapping[province], {}).get("deaths", 0) + data[1],
                "recovered": stats.get(mapping[province], {}).get("recovered", 0)
                + data[2],
            }
        elif province == "м.":
            stats["Kiev Oblast"] = {
...
def get_covid_country_data(country: str) -> dict:
    ...
    if country == "Ukraine":
        ...
        if res.status_code == 200:
            ...
            cache.set(
                f"covid_19_{country}_last_modified", datetime.now().isoformat(), None
            )
        ...
        if res.status_code == 200:
            ...
            cache.set(
                f"covid_19_{country}_last_modified", res["data"]["lastChecked"], None
            )
